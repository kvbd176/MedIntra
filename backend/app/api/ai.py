...
def chat(
    ...
    if intent == "low_stock":
        ...
        result = orchestrator.customer_agent.customer_summary(route["phone"])
    elif intent == "distributor_medicines":
        result = orchestrator.distributor_agent.distributor_medicines(route["name"])
    else:
        result = {"message": "Unable to process query"}
    answer = generate_answer(question=query,data=result)
    return {
        "answer": answer,
        "data": result
    }